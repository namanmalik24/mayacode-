...
import { UserCircleIcon, SparklesIcon } from "../../components/icons/icons"; // Assuming SparklesIcon for AI
      ...
      <div className={`flex items-start space-x-2 max-w-[85%] sm:max-w-[75%]`}>
        ...
        <div
          className={`
            ...
            ${
              isUser
                ? "bg-accent text-white order-1"
                : isError
                ? "bg-status-red/20 text-status-red"
                : "bg-card border border-border-color text-text-primary"
            }
          `}
        >
          {message.isLoading && message.text === "" ? (
            <span className="italic text-sm">
              {assistantLabels.typingIndicator || "Maya is typing..."}
            </span>
          ) : (
