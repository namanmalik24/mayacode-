...
const LeftSlidingPanel: React.FC<LeftSlidingPanelProps> = ({
  isOpen,
  onClose,
  title,
  children,
  ...
  return (
    <>
      ...
      >
        {/* Panel Header */}
        <div className="flex justify-between items-center p-4 sm:p-5 border-b border-border-color bg-card-header rounded-t-lg">
          {" "}
          {/* Added rounded-t-lg */}
          <h2
            id={`panel-title-${title.replace(/\s+/g, "-")}`}
        ...
        {/* Panel Content */}
        <div className="flex-grow overflow-y-auto bg-card p-4 sm:p-6 rounded-b-lg">
          {" "}
          {/* Added rounded-b-lg */}
          {children}
        </div>
