...
const DetailSection: React.FC<{
  ...
  <div className={`mb-4 ${className}`}>
    ...
    <div className="text-sm text-text-primary bg-background p-3 rounded-md border border-border-color">
      {children}
    </div>
  </div>
);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={detailsTitles.title}>
      <div className="space-y-3">
        <div
          className={`p-4 rounded-lg border-l-4 ${statusClasses.border} ${statusClasses.bg} flex items-center space-x-3 mb-4`}
        >
          ...
          <div>
            ...
            <p className={`text-sm font-medium ${statusClasses.text}`}>
              {statusText}
            </p>
          </div>
        </div>

        {action.status === ProgressStatus.InProgress &&
          action.progressValue !== undefined && (
            <DetailSection title={detailsTitles.progress}>
              ...
              <p className="text-xs text-text-secondary mt-1">
                {action.progressValue}% complete
              </p>
            </DetailSection>
          )}

        <DetailSection title={detailsTitles.what}>
          <p>{action.details.what}</p>
        </DetailSection>
        <DetailSection title={detailsTitles.why}>
          <p>{action.details.why}</p>
        </DetailSection>
        <DetailSection title={detailsTitles.preparedByMaya}>
          <p>{action.details.preparedByMaya}</p>
        </DetailSection>
        <DetailSection title={detailsTitles.stillMissing}>
          <p>{action.details.stillMissing}</p>
        </DetailSection>

        {action.details.links && action.details.links.length > 0 && (
          <DetailSection title={detailsTitles.helpfulLinks}>
            <ul className="list-disc list-inside space-y-1">
              {action.details.links.map(
                (link: ActionItemLink, index: number) => (
                  <li key={index}>
                    <a
                      href={link.url}
                      target="_blank"
        ...
        {action.details.deadlines && (
          <DetailSection title={detailsTitles.deadlines}>
            <p className={`font-medium ${statusClasses.text}`}>
              {action.details.deadlines}
            </p>
          </DetailSection>
        )}
