...
const SuggestedActionsSection: React.FC<SuggestedActionsSectionProps> = ({
  ...
  const getSectionTitle = () => {
    ...
    if (activeFilter === "completed")
      return T.suggestedActions.completedTasksTitle;
  ...
  if (actions.length === 0) {
    let emptyMessage =
      emptyDashboardMessages.generic ||
      "No actions to display for this category.";
    if (activeFilter === null) {
      emptyMessage =
        emptyDashboardMessages.noInProgressTasks ||
        "No tasks are currently in progress or pending. Well done!";
    } else if (activeFilter === "nextSteps") {
      emptyMessage =
        emptyDashboardMessages.noNotStartedTasks ||
        "You're all caught up with your next steps!";
    } else if (activeFilter === "completed") {
      emptyMessage =
        emptyDashboardMessages.noCompletedTasks ||
