...
import { CheckCircleIcon } from "../icons/icons"; // For completed steps
    ...
    <nav aria-label="Workflow Progress" className="py-4 px-2 sm:px-0">
      <ol className="flex flex-wrap items-center justify-center space-y-2 sm:space-y-0 sm:space-x-2 md:space-x-4">
        {steps.map((step, index) => {
          ...
          const stepName =
            T.workflowSteps[step.id] ||
            step.nameKey.substring(step.nameKey.lastIndexOf(".") + 1);

          return (
            <li
              key={step.id}
              className="flex-shrink-0 sm:flex-1 w-full sm:w-auto"
            >
              <button
