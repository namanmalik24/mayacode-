...
import { ArrowUpTrayIcon, ChevronDownIcon, ChevronUpIcon } from "../../components/icons/icons";
...
const SidebarCombinedMenu: React.FC<SidebarCombinedMenuProps> = ({
  ...
  const exportOptionLabels = T.exportProfileOptions || {
    pdf: "PDF",
    json: "JSON",
    text: "TXT",
  };
  const pdfAlert =
    T.pdfExportAlert || "PDF export simulation: Downloading as TXT.";

  const handleExport = (format: "PDF" | "JSON" | "TXT") => {
    let content = "";
    let filename = `${
      userProfile.name || "user"
    }_profile.${format.toLowerCase()}`;
    let mimeType = "";
  ...
  const selectedLanguageName =
    T.languageNames[currentLanguage] || currentLanguage;
